...
def lnprob(params, time, population, ynoise):
	...
	model = L * nm.exp((time - time[0])/tau)
...
p0 = 10.0*nm.random.rand(ndim * nwalkers).reshape((nwalkers, ndim)) + \
     nm.transpose(nm.vstack((L_guess*nm.ones(nwalkers),tau*nm.ones(nwalkers))))
...
time_array_for_model = nm.arange(1776,2040.0,1.0)
model_best = L * nm.exp((time_array_for_model - time[0])/tau)

# calculate the chi-squared for the best fit
model_best_for_chi2 = L * nm.exp((time - time[0])/tau)
...
pylab.plot(time_array_for_model,model_best,'k',label='Best-Fit Exponential Model')
pylab.xlabel('Time [CE]')
pylab.ylabel('US Population [millions]')
pylab.legend(loc='upper left')
pylab.title('Exponential Model: Best chi-squared is ' + str(chi2_best))